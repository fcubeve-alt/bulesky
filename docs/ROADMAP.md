...
- **选了 OpenAI `gpt-4o-mini-tts`,没选 ElevenLabs**。理由两条:①它接受一句大白话的表演指示并且真的照做——`src/tts.js` 里 `DELIVERY.pain` 写的是"像在安慰一个坐在你旁边、正在难过的朋友,不急不躁,低而温暖,带一点悲伤,在人会自然换气的地方留白;绝不明亮、绝不轻快";②便宜约 6 倍(一条 400 字的悄悄话约 $0.006)。**倾诉和许愿用两套不同的指示**——它们不该用同一种语气念。
