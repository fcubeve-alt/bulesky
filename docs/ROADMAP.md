...
- **成本天然封顶,不需要限流**:`/api/voice/<id>` 把音频按「文本 + 音色 + 表演指示」的 SHA-256 缓存进 D1 的 `voice` 表。同一条悄悄话**这辈子只付一次钱**,之后无限次重播都是免费读库。悄悄话是有限的,所以总账也是有限的。改了 `RECIPE`/音色/指示 → hash 变了 → 自动重新生成,不用手动清缓存。
