...
## 10. 2026-08-29 晚:上传全对,但一条都没人听见

现象:`POST /api/voice/backfill` 全部成功,`made=84`,每条 hash 都对得上 ——
**而 Listen 播出来的还是随机的默认 TTS。**

两个原因,叠在一起,而且**两个都不会报错**。

### 10a. ⚠️ 标签和字节对不上 —— 这是主因

`ffmpeg -c:a aac` 写进 `.m4a`,产物是 **MP4 容器**。上传时标的是 `audio/aac`,
而那是**裸 ADTS 流**的 MIME。标签和字节不符,**Safari 拒绝解码**。

然后 `app.js` 做了完全正确的事:

```js
audio.onerror = () => giveUpToBrowser(`cannot decode ${blob.type}`);
// → speakInBrowser(text)  = 手机自带的 speechSynthesis
```

手机自带的音色每次可能不一样 —— 那就是「随机默认 TTS」。

**整条链上没有任何一处报错。** 上传 200,存储正确,服务端返回 200 + audio/*,
客户端优雅降级。八十四条完美地失败了,而且没有任何地方看得出来。

**现在:MIME 由字节决定,不由调用方决定。** `sniffAudioMime()`(`src/voice-store.js`)
在**上传时**认出容器并存正确的类型,在**读取时**也再认一次 —— 所以**已经传上去的
八十四条不用重做**,下一次播放就是对的。

### 10b. 键改成按 whisper id

`brandVoiceKey(id)` → `aya-id-<id>`。

按文本 hash 存对**机器音**是对的(两条一样的悄悄话共用一次合成和一次账单)。对**品牌音**
是错的:每条都在 Owner 的机器上单独生成,省不下任何东西,而**只要上传端和读取端算出的
文本有一丁点差别,就是一次静默的 miss,哪里都不报错**。id 不会漂。

- `/api/voice/{id}` **先读 id 键,再读旧的文本 hash 键** —— 已经传上去的那批照常播,
  不用为了换键去 GPU 上重念一遍。
- `GET /api/voice/backfill` 返回的 `hash` 也改成 id 键,否则上传端的账本永远对不上,
  每次运行都会把整片天空重读一遍。
- `alreadyRead()` 不再比对文本 hash —— 键已经不看文本了,而作者发布后改不了字。

### 10c. 顺带:`max-age` 从 86400 降到 600

这个 URL 的音频**不是不变的**:今天机器音,明天品牌音。一天的缓存意味着**听过一次的手机
会继续放一整天机器音**,而且没有办法要新的。十分钟仍然省掉重复听的下载,又能让新朗读真的
到得了。R2 出站免费。

### 10d. 回归测试

`tools/revoice-test.mjs`:
- 用 `content-type: audio/aac` 传一段 **MP4 字节**,断言存的是 `audio/mp4`,
  并且 Listen 返回浏览器解得开的类型;
- 用**旧的文本 hash 键**直接写一条,断言 Listen 仍然播得出来。

---

## 11. 「上传不覆盖、越传越多」—— 其实是换键那一刻的错觉

现象:重传之后 `made` 从 **84 涨到 169**,看起来像是每次上传都在新增。

**并不是。同一个 id 传两次,写的是同一个 R2 key,`put` 直接替换。**
涨上去是因为**两批之间键的定义变了**:

- 前 84 条:传于换键之前,落在 `aya-<文本 sha256>` 上
- 后 85 条:传于换键之后,落在 `aya-id-<id>` 上
- 而 `made` 数的是 `voice/aya-` 前缀下的**全部**对象 —— 同一批悄悄话被在两个货架上各数了一遍

Listen **先读 id 键**,所以听到的一直是新的那批。旧的 84 条是永远读不到的死重量。

### 11a. 计数改成两个数

`GET /api/voice/backfill` 现在同时返回:

| 字段 | 含义 |
|---|---|
| `made` | 当前键(`aya-id-`)下的朗读数 —— **一条悄悄话一条,重传不会让它变大** |
| `legacy` | 还压在旧文本 hash 键上的残留 —— 没人读,只占地方 |

把它们合成一个数,正是 84→169 那次误会的全部原因。

### 11b. 两条清理路径

1. **每次重传自我清理。** POST 写完 id 键之后,顺手删掉这条悄悄话的旧副本
   (`dropLegacy`)。所以只要一条悄悄话被重念过,它的旧版就没了。
2. **`GET /api/voice/backfill?sweep=1&after=&limit=`** —— 给那些不会再被重念的。
   `tools/revoice.mjs` 在每次上传跑完之后会自动分页扫一遍。

   ⚠️ **分页是必须的,不是讲究。** Worker 对单次请求能发出的外部调用数有上限,
   而扫一条悄悄话要两次存储调用。一次扫完整库在测试里能过,在线上会死。

   ⚠️ **只删「已经有当前版本」的那条的旧副本。** 一条悄悄话如果只剩旧键上那一份,
   它就是唯一能播的东西,扫描必须放过它 —— 否则清理会把声音清没。

### 11c. 回归测试

- 同一个 id 传两次:`made` 仍然是 1,而且存的是**第二次**那份字节;
- 只有旧键副本时,计数报成 `legacy` 而不是 `made`,并且 **sweep 不动它**;
- 重传之后旧副本自动消失;
- 对不会再重念的,sweep 清掉旧的,留下一条悄悄话一条朗读。
